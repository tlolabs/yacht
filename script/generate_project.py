...
icon_ref = add('icon-ref', isa='PBXFileReference', lastKnownFileType='folder.iconcomposer.icon', path='assets/icons/YACHT.icon', sourceTree='SOURCE_ROOT')
icon_build = add('icon-build', isa='PBXBuildFile', fileRef=icon_ref)
...
phases['app'].append(add('app-resources', isa='PBXResourcesBuildPhase', buildActionMask=2147483647, files=[license_build, icon_build], runOnlyForDeploymentPostprocessing=0))
groups.extend([license_ref, icon_ref])
...
app_configs=configs('app', {'PRODUCT_NAME':'YACHT', 'EXECUTABLE_NAME':'YachtApp', 'PRODUCT_MODULE_NAME':'YachtNativeApp', 'PRODUCT_BUNDLE_IDENTIFIER':'com.local.yacht.csvhtmltranslator', 'INFOPLIST_FILE':'platform/macos/Support/Info.plist', 'ASSETCATALOG_COMPILER_APPICON_NAME':'YACHT', 'LD_RUNPATH_SEARCH_PATHS':['$(inherited)', '@executable_path/../Frameworks']})
